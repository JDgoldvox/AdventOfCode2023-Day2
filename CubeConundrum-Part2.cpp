...
bool PlayRound(const string& line, const int& start, const int& end, int& powerRed, int& powerGreen, int& powerBlue);
...
int main() {
	...
	if (myFile.is_open()) {
		while (getline(myFile, line)) {
			...
			int powerRed = 0, powerGreen = 0, powerBlue = 0;
			...
			while (currentPosition < (line.size() - 1)) {
				...
				//play the round
				PlayRound(line, currentPosition, currentEndPosition, powerRed, powerGreen, powerBlue);
				currentPosition = currentEndPosition + 1;
			}

			/*cout << "red: " << powerRed << endl;
			cout << "green: " << powerGreen << endl;
			cout << "blue: " << powerBlue << endl;*/

			playableIDs.push_back(powerRed * powerGreen * powerBlue);
...
bool PlayRound(const string& line, const int& start, const int& end, int& powerRed, int& powerGreen, int& powerBlue) {
	...
	//check if any power cubes need to be added (biggest number)
	if (powerRed < red) powerRed = red;
	if (powerGreen < green) powerGreen = green;
	if (powerBlue < blue) powerBlue = blue;
